...
public class ShootingEnemyStates : MonoBehaviour
{
    ...
    private void Patrol() //Finch� � in patrol, si sposta e si comporta di conseguenza al player
    {
    ...
    private void Aim() // In aim, enemy si prende del tempo per prendere bene la mira prima di sparare, ma se player si avvicina troppo spara senza pensarci
    {
        if(aimTimer > aimDuration)
        {
            ...
            if (playerDistance > aimingDistance)
            {
                rangedEnemyState = RangedEnemyStates.Patrol; // In questo caso se player � troppo lontano, enemy non lo insegue ma si sposta in un'altra posizione, si pu� cambiare volendo
                inAim = false;
                return;
            }
        }
